...
public class FroglinModel extends HumanoidModel<FroglinRenderState>
{
...
//	}
	...
	@Override
	public void setupAnim(FroglinRenderState froglin) //FroglinEntity froglin, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch)
	{
		if (froglin.pose == Pose.CROUCHING)
		{
...
//		this.copyBaseBone(currentModel, model -> model.leftLeg);
		...
		this.head.yRot = froglin.yRot * ((float) Math.PI / 180F);
		if (froglin.swimAmount > 0.0F)
		{
			if (isSwimming)
			{
				this.head.xRot = Mth.rotLerpRad(froglin.swimAmount, this.head.xRot, (-(float) Math.PI / 4F));
			}
			else
			{
				this.head.xRot = Mth.rotLerpRad(froglin.swimAmount, this.head.xRot, froglin.xRot * ((float) Math.PI / 180F));
			}
		}
		else
		{
			this.head.xRot = froglin.xRot * ((float) Math.PI / 180F);
		}
		
		this.rightArm.xRot += Mth.cos(froglin.walkAnimationPos * 0.6662F + (float) Math.PI) * 2.0F * froglin.walkAnimationSpeed * 0.5F / froglin.speedValue;
		this.leftArm.xRot += Mth.cos(froglin.walkAnimationPos * 0.6662F) * 2.0F * froglin.walkAnimationSpeed * 0.5F / froglin.speedValue;
		this.rightLeg.xRot += Mth.cos(froglin.walkAnimationPos * 0.6662F) * 1.4F * froglin.walkAnimationSpeed / froglin.speedValue;
		this.leftLeg.xRot += Mth.cos(froglin.walkAnimationPos * 0.6662F + (float) Math.PI) * 1.4F * froglin.walkAnimationSpeed / froglin.speedValue;
		if (froglin.isPassenger)
		{
...
//		}
		...
		this.setupAttackAnimation(froglin);
//		if (this.isSneak)
//		{
...
//		}
		...
		AnimationUtils.bobArms(this.rightArm, this.leftArm, froglin.ageInTicks);
		if (froglin.swimAmount > 0.0F)
		{
			float limbSwingMod26 = froglin.walkAnimationPos % 26.0F;
			HumanoidArm handside = froglin.attackArm;
			float rightArmSwing = handside == HumanoidArm.RIGHT && froglin.attackTime > 0.0F ? 0.0F : froglin.swimAmount;
			float leftArmSwing = handside == HumanoidArm.LEFT && froglin.attackTime > 0.0F ? 0.0F : froglin.swimAmount;
			if (limbSwingMod26 < 14.0F)
			{
				this.leftArm.xRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.xRot, 0.0F);
				this.rightArm.xRot = Mth.lerp(rightArmSwing, this.rightArm.xRot, 0.0F);
				this.leftArm.yRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.yRot, (float) Math.PI);
				this.rightArm.yRot = Mth.lerp(rightArmSwing, this.rightArm.yRot, (float) Math.PI);
				this.leftArm.zRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.zRot,
			...
			else if (limbSwingMod26 >= 14.0F && limbSwingMod26 < 22.0F)
			{
				...
				this.leftArm.xRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.xRot, ((float) Math.PI / 2F) * swingLerp);
				this.rightArm.xRot = Mth.lerp(rightArmSwing, this.rightArm.xRot, ((float) Math.PI / 2F) * swingLerp);
				this.leftArm.yRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.yRot, (float) Math.PI);
				this.rightArm.yRot = Mth.lerp(rightArmSwing, this.rightArm.yRot, (float) Math.PI);
				this.leftArm.zRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.zRot, 5.012389F - 1.8707964F * swingLerp);
			...
			else if (limbSwingMod26 >= 22.0F && limbSwingMod26 < 26.0F)
			{
				...
				this.leftArm.xRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.xRot, ((float) Math.PI / 2F) - ((float) Math.PI / 2F) * swingLerp);
				this.rightArm.xRot = Mth.lerp(rightArmSwing, this.rightArm.xRot, ((float) Math.PI / 2F) - ((float) Math.PI / 2F) * swingLerp);
				this.leftArm.yRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.yRot, (float) Math.PI);
				this.rightArm.yRot = Mth.lerp(rightArmSwing, this.rightArm.yRot, (float) Math.PI);
				this.leftArm.zRot = Mth.rotLerpRad(leftArmSwing, this.leftArm.zRot, (float) Math.PI);
				this.rightArm.zRot = Mth.lerp(rightArmSwing, this.rightArm.zRot, (float) Math.PI);
			}

			this.leftLeg.xRot = Mth.lerp(froglin.swimAmount, this.leftLeg.xRot, 0.3F * Mth.cos(froglin.walkAnimationPos * 0.33333334F + (float) Math.PI));
			this.rightLeg.xRot = Mth.lerp(froglin.swimAmount, this.rightLeg.xRot, 0.3F * Mth.cos(froglin.walkAnimationPos * 0.33333334F));
			this.head.xRot = 0F;
		}
	}
	...
	@Override
	protected void setupAttackAnimation(FroglinRenderState froglin)
	{
		if (!(froglin.attackTime <= 0.0F))
		{
			HumanoidArm mainHand = froglin.attackArm;
			ModelPart mainArmRenderer = this.getArm(mainHand);
			float swingProgress = froglin.attackTime;
...
//			this.bipedLeftArm.rotationPointX += MathHelper.cos(this.bipedBody.rotateAngleY) * 5.0F;
			...
			swingProgress = 1.0F - froglin.attackTime;
			...
			float headRadians = Mth.sin(froglin.attackTime * (float) Math.PI) * -(this.head.xRot - 0.7F) * 0.75F;
			mainArmRenderer.xRot = (float) (mainArmRenderer.xRot - (swingRadians * 1.2D + headRadians));
			mainArmRenderer.yRot += this.body.yRot * 2.0F;
			mainArmRenderer.zRot += Mth.sin(froglin.attackTime * (float) Math.PI) * -0.4F;
		}
	}
